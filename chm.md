...
create_dir(回测/交易)	set_parameters(回测/交易)
参数
...
get_CCI(回测/交易)	create_dir(回测/交易)	get_opt_objects(研究/回测/交易(期权))
get_opt_last_dates(研究/回测/交易(期权))	get_opt_contracts(研究/回测/交易(期权))	get_contract_info(研究/回测/交易(期权))
...
debt_to_stock_order(交易)	get_user_name(回测/交易)	get_research_path(回测/交易)
get_marginsec_stocks(交易)	get_margincash_stocks(交易)	debt_to_stock_order(交易)
...
get_covered_unlock_amount(交易(期权))	buy_open(交易(期权))	sell_close(交易(期权))
sell_open(交易(期权))	buy_close(交易(期权))	open_prepared(交易(期权))
...
可调用接口
get_trades_file(回测)	get_stock_info(研究/回测/交易)	get_open_orders(回测/交易)
...
get_opt_contracts(研究/回测/交易(期权))	get_contract_info(研究/回测/交易(期权))	set_parameters(回测/交易)
参数
context: Context对象，存放有当前的账户及持仓信息；
...
def handle_data(context, data):
    # 获取当前股票池逐笔成交数据
    transcation = get_individual_transcation()
    log.info(transcation)

    # 获取指定股票列表逐笔成交数据
    transcation = get_individual_transcation(["000002.SZ", "000032.SZ"])
    log.info(transcation)

    # 获取成交量
    if transcation is not None:
        business_amount = transcation["000002.SZ"]["business_amount"]
...
def handle_data(context, data):
    if not g.flag:
        ...
        # 以35保护限价按最优五档即时成交剩余转限价买入100股
        margin_trade(g.security, 100, market_type=1, limit_price=35)
        # 按全额成交或撤单买入100股
...
2、入参的security非str非list类型，返回空字典
...
修复交易场景中获取当日K线14:58、14:59分价格为0问题；
...
get_individual_transcation()新增返回buy_no(叫买方编号)、sell_no(叫卖方编号)、trans_flag(成交标记)、trans_identify_am(盘后逐笔成交序号标识)、channel_num(成交通道信息)字段；
...
get_individual_entrust()和get_individual_transcation()返回内容中hq_px字段值缩小1000倍，返回为真实价格；
...
修复get_individual_entrust()和get_individual_transcation()中stocks入参代码数量大于50只时返回数据缺失问题；
run_interval()和run_daily()中新增支持get_sort_msg()调用；
get_snapshot()新增返回business_amount_in(内盘成交量)、business_amount_out(外盘成交量)字段；
交易场景中buy_open()、sell_close()、sell_open()、buy_close()新增支持期权业务；
on_trade_response()新增接收撤单的成交主推，详见接口说明注意事项；
Portfolio账户对象新增支持期权业务，字段详见帮助说明；
Position持仓对象新增支持期权业务，字段详见帮助说明；
新增get_contract_info()获取期权合约信息；
新增get_opt_objects()获取期权标的列表；
新增get_opt_last_dates()获取期权标的到期日列表；
新增get_opt_contracts()获取期权标的对应合约列表；
新增get_covered_lock_amount()获取期权标的允许备兑锁定数量；
新增get_covered_unlock_amount()获取期权标的允许备兑解锁数量；
新增option_covered_lock()期权标的备兑锁定；
新增option_covered_unlock()期权标的备兑解锁；
...
由于Keras库2.3.1版本与TensorFlow库不兼容，Keras降为2.2.4版本。
