...
**Goal:** 创建 `strategies/george_limit_up_sell.py`，监控所有持仓股票，涨停后卖一封单 < 5000手时以涨停价×98%卖出。

**Architecture:** 单一策略文件，使用 PTrade 兼容 API。交易模式用 run_interval 秒级轮询，回测模式用 handle_data 分钟级驱动。核心逻辑：获取持仓 → check_limit 判断涨停 → offer_grp 检查封单量 → order 限价卖出。

**Tech Stack:** Python 3.10+, PTrade 兼容 API (SimTradeLab)

**参考文件:**
- `strategies/george_omo_limit_single.py` — 现有 george 策略模板（持仓读取、get_snapshot、offer_grp 用法）
- `strategies/george_sell_strage.py` — 现有卖出策略（get_position、sell 逻辑）
- `strategies/hit_limit_single.py` — check_limit 用法、is_trade/run_interval/回测兼容模式
- `docs/superpowers/specs/2026-05-18-limit-up-sell-strategy-design.md` — 设计文档
...
**Files:**
- Create: `strategies/george_limit_up_sell.py`
...
def set_params():
    ...
    g.holdings = []                     # 当前持仓股票列表
    g.last_check_date = None            # 上次检查日期（防重复）
...
Run: `python -c "import ast; ast.parse(open('strategies/george_limit_up_sell.py', encoding='utf-8').read()); print('Syntax OK')"` (use workdir: `E:\P_python\kay\SimTradeLab_old`)
...
```bash
git add strategies/george_limit_up_sell.py docs/superpowers/specs/2026-05-18-limit-up-sell-strategy-design.md docs/superpowers/plans/2026-05-18-limit-up-sell-strategy.md
git commit -m "feat: add limit-up sell strategy - monitor holdings, sell when sell-1 pending < 5000 lots at cage bottom price (up_px × 98%)"
```
